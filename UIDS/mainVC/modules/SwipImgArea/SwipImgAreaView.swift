...
class SwipImgAreaView: BaseModuleView, UICollectionViewDelegate, UICollectionViewDataSource, UIScrollViewDelegate  {
    ...
    // MARK: 纯文本
    open class func llCycleScrollViewWithTitles(frame: CGRect, backImage: UIImage? = nil, titles: Array<String>? = [], didSelectItemAtIndex: LLdidSelectItemAtIndexClosure? = nil) -> SwipImgAreaView {
        let llcycleScrollView: SwipImgAreaView = SwipImgAreaView.init(frame: frame)
        // Nil
        llcycleScrollView.titles = []
    ...
    func setupPageControl() {
        ...
        if customPageControlStyle == .fill {
            customPageControl = SwipImgAreaPageControl.init(frame: CGRect.zero)
        ...
        if customPageControlStyle == .pill {
            customPageControl = SwipImgAreaPillPageControl.init(frame: CGRect.zero)
        ...
        if customPageControlStyle == .snake {
            customPageControl = SwipImgAreaSnakePageControl.init(frame: CGRect.zero)
        ...
        if customPageControlStyle == .none || customPageControlStyle == .system || customPageControlStyle == .image {
            if pageControlPosition == .center {
                ...
                if pageControlPosition == .left {
                    pageControl?.frame = CGRect.init(x: -(UIScreen.main.bounds.width - (pointSize?.width)! - pageControlLeadingOrTrialingContact) * 0.5, y: self.height-pageControlBottom, width: UIScreen.main.bounds.width, height: 10)
                }else{
                    pageControl?.frame = CGRect.init(x: (UIScreen.main.bounds.width - (pointSize?.width)! - pageControlLeadingOrTrialingContact) * 0.5, y: self.height-pageControlBottom, width: UIScreen.main.bounds.width, height: 10)
                }
            }
    ...
    open func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell: SwipImgAreaCell = collectionView.dequeueReusableCell(withReuseIdentifier: identifier, for: indexPath) as! SwipImgAreaCell
        // Setting
        cell.titleFont = font
