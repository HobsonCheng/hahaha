...
class DS2RefreshHeader: UIView, ESRefreshProtocol, ESRefreshAnimatorProtocol{
    ...
    private let imageView: UIImageView = {
        ...
        imageView.image = UIImage.init(named: "icon_pull_animation_1.png")
        return imageView
    }()
    ...
    public func refreshAnimationBegin(view: ESRefreshComponent) {
        ...
        UIView.animate(withDuration: 0.2, delay: 0, options: .curveLinear, animations: {
            ...
            var images = [UIImage]()
            for idx in 1 ... 8 {
                if let aImage = UIImage(named: "icon_shake_animation_\(idx).png") {
    ...
    public func refresh(view: ESRefreshComponent, stateDidChange state: ESRefreshViewState) {
        ...
        case .pullToRefresh:
            var images = [UIImage]()
            for idx in 1 ... 5 {
                if let aImage = UIImage(named: "icon_pull_animation_\((5 - idx + 1)).png") {
                    images.append(aImage)
                }
            }
        ...
        case .releaseToRefresh:
            var images = [UIImage]()
            for idx in 1 ... 5 {
                if let aImage = UIImage(named: "icon_pull_animation_\(idx).png") {
                    images.append(aImage)
                }
            }
            imageView.animationDuration = 0.2
            imageView.animationRepeatCount = 1
            imageView.animationImages = images
            imageView.image = UIImage.init(named: "icon_pull_animation_5.png")
            imageView.startAnimating()
            break
        default:
            break
